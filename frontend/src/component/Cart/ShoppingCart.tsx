...
import { addOrderCart, Order, updateOrderStatus } from "../../store/orderSlice";
...
const ShoppingCart: React.FC<PropCart> = ({ carts, setToggle }) => {
  ...
  const handleQuantityChange = async (id: string, delta: number) => {
    if (delta > 0) {
      await dispatch(
        updateOrderStatus({ orderId: id, quantity: 1, increase: true })
      );
      setToggle((prev) => !prev);
    } else {
      await dispatch(
        updateOrderStatus({ orderId: id, quantity: 1, increase: false })
      );
      setToggle((prev) => !prev);
    }
  };
  ...
  const handleRemoveItem = async (id: string) => {
    await dispatch(updateOrderStatus({ orderId: id, isDelete: true }));
    setToggle((prev) => !prev);
  };
  ...
  const handleOrdered = async () => {
    await dispatch(addOrderCart(carts));
    navigate("/checkout");
  };
  return (
    <Fragment>
